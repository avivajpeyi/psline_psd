...
def lprior(args: LnlArgs):
    ...
    φ, δ, alph = args.φ, args.δ, args.alph
    ...
    log_prior += uniform.logpdf(alph, 0, 1)
    return log_prior
...
def lpost(args: LnlArgs):
    ...
    loglike = spline_model.lnlikelihood(
        data=data, weights=w, alph=args.alph, lnparam=args.param
    )  # args.param[1:] for AR
